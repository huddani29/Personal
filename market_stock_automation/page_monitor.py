...
live_period = st.sidebar.selectbox("Időtartam (Átírva fix Pro-ra):", options=["Fix Biztonsági Mód 🔐"], index=0, key="live_p")
refresh_rate = st.sidebar.slider("Frissítés (mp):", min_value=10, max_value=300, value=30, key="refresh_rate_slider")

# Meghívjuk a módosított háttérmotort
...
if data is not None and not data.empty:
    ...
    if not data_clean.empty:
        ...
        if "BUY" in current_signal: st.session_state.send_discord_message(f"🚨 **ÉLES BOT VÉTELI SZIGNÁL TRIGERELVE!** `{live_ticker}` -> `${latest_price:.2f}`")
        ...
        fig.add_trace(go.Scatter(x=data_clean.index, y=data_clean['SMA_20'], line=dict(color='orange', width=1), name="SMA 20 (Rövid)"), row=1, col=1)
        
        # ÚJ: Kirajzoljuk a vastag piros SMA 200 hosszú távú trendvonalat
        if 'SMA_200' in data_clean.columns:
            fig.add_trace(go.Scatter(x=data_clean.index, y=data_clean['SMA_200'], line=dict(color='red', width=2), name="SMA 200 (FŐ TREND)"), row=1, col=1)
            
        # ÚJ: Kirajzoljuk a SuperTrend dinamikus görbéjét
        if 'ST_Value' in data_clean.columns:
            # Színválasztás az irány alapján (1=zöld, -1=piros)
            st_colors = ['#2ecc71' if x == 1 else '#e74c3c' for x in data_clean['ST_Direction']]
            fig.add_trace(go.Scatter(x=data_clean.index, y=data_clean['ST_Value'], line=dict(color='#3498db', width=1.5, dash='dot'), name="SuperTrend Vonal"), row=1, col=1)
